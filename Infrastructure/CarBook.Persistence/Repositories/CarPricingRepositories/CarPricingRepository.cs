...
namespace CarBook.Persistence.Repositories.CarPricingRepositories
{
    public class CarPricingRepository : ICarPricingRepository
	{
		...
		{
			var values = _context.CarPricings.Include(x => x.Car).ThenInclude(y => y.Brand).Include(x => x.Pricing).Where(z => z.PricingID == 2).ToList(); // 1=hourly, 2=daily, 3=weekly, 4=monthly
			return values;
		}
		...
		var values = from x in _context.CarPricings
						...
						select new
						{
							...
							WeekPrice = g.Where(y => y.CarPricingID == 3).Sum(z => z.Amount),
